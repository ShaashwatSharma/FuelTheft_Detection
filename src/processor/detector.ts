  ...
  for (const sensor of sensors) {
    try {
      ...
      for (const curr of pending) {
        ...
        const fuelDiff = fuelNow - effectivePrevFuel; // Negative = fuel consumed/dropped  
        ...
        const deltaLitres = prevFuel != null ? (fuelNow - prevFuel) : 0; // Same as fuel_diff calculation
        const deltaStr = (deltaLitres >= 0 ? '+' : '') + deltaLitres.toFixed(2) + 'L';
